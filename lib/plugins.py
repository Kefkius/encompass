...
class Plugins(DaemonThread):
    ...
    def load_plugins(self):
        for loader, name, ispkg in pkgutil.iter_modules([self.pkgpath]):
            ...
            if not d.get('requires_wallet_type') and self.config.get_above_chain('use_' + name):
                try:
    ...
    def load_plugin(self, name):
        full_name = 'encompass_plugins.' + name + '.' + self.gui_name
    ...
    def enable(self, name):
        self.config.set_key_above_chain('use_' + name, True, True)
        p = self.get(name)
        if p:
            return p
    ...
    def disable(self, name):
        self.config.set_key_above_chain('use_' + name, False, True)
        p = self.get(name)
        if not p:
            return
